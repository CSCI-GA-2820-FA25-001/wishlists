...
class WishlistItems(db.Model, PersistentBase):
    ...
    wishlist_id = db.Column(db.Integer, db.ForeignKey('wishlists.id', ondelete='CASCADE'), primary_key=True)
    ...
    def deserialize(self, data: dict) -> None:
        ...
        try:
            if type(data['product_id']) is not int:
                raise DataValidationError('product_id must be an integer')
    ...
    def find_by_wishlist_and_product(cls, wishlist_id: int, product_id: int):
        ...
        return cls.query.filter(cls.wishlist_id == wishlist_id, cls.product_id == product_id).first()
    
    @classmethod
    def find_last_position(cls, wishlist_id: int):
        """ Find the last position number in a given wishlist """
        item = cls.query.filter(cls.wishlist_id == wishlist_id).order_by(cls.position.desc()).first()
