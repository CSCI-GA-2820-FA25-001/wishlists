...
def step_impl(context: Any, message: str) -> None:
    ...
    logging.info(f"Expected '{message}' in title, but got: '{actual_title}'")
    assert (
        message in actual_title
    ), f"Expected '{message}' in title, but got: '{actual_title}'"
...
# prefixed by ID_PREFIX so the Name field has an id='name'
